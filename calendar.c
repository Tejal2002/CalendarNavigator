...
int main()
{
    ...
    struct Node* stacktop = create(""); // "" indicates the base of the Stack

    //calender main//

    char month2[][20]={ "","January ","February ","March ","April ","May ","June ","July ","August ","September ","October ","November ","December "};
    int nmonth,nyr,ch;
    enteryear:
    while(1)
    {
        ...
        sprintf(str1,"%s",month2[nmonth]);
        sprintf(str2,"%d",nyr);

        strcpy(str1 , strcat(str1, str2));
    
        stacktop = push(stacktop, str1);
        if(nyr<1945)
        {
